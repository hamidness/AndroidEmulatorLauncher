...
const openNewWindow = (newWindowSpec: NewWindowSpec) => {
    ...
    const x = mainWindow != null ? mainWindow.getBounds().x + mainWindow.getBounds().width / 2 - width / 2 : 0;
    const y = mainWindow != null ? mainWindow.getBounds().y + mainWindow.getBounds().height / 2 - height / 2 : 0;
...
function registerCloseCreateAvdWindowListener() {
    ipcMain.on("close-create-avd-window", () => {
